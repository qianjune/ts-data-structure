...
import BuriedPointDb from "@micro-services/buried-point-service/src/db/buriedPoint";

const placeholder = "BuriedPoint";
const responseMsg = ResponseMsg(placeholder);
class BuriedPointManager implements CommonManager {
  /**
   * 获取详情（私有）
   * @param id
   * @param config
  ...
  async _getInfo(
    ...
    const item = await BuriedPointDb.findOne({
      where,
    });
    if (!item) {
  ...
  async create(data: any): Promise<ManagerResponse<any>> {
    ...
    const result = await BuriedPointDb.create(data);
    if (result) {
  ...
  async edit(data: any): Promise<ManagerResponse<any>> {
    ...
    const result = await BuriedPointDb.update(updateData, {
      where: {
        id,
      },
    });
  ...
  async del(id: number): Promise<ManagerResponse<any>> {
    ...
    return await sequelize.transaction(async (t: any) => {
      const result = await BuriedPointDb.destroy({
        where: {
          id,
        },
      });
  ...
  async getList?(
    ...
    return await sequelize.transaction(async (t: any) => {
      ...
      const result = await BuriedPointDb.findAndCountAll({
        ...listParams,
      });
      const { count, rows } = result;
      const BuriedPointList = rows.map((row: any) => {
...
export default BuriedPointManager;
