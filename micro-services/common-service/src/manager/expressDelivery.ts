...
import ExpressDeliveryDb from "@micro-services/common-service/src/db/expressDelivery";

const placeholder = "ExpressDelivery";
const responseMsg = ResponseMsg(placeholder);
class ExpressDeliveryManager implements CommonManager {
  /**
   * 获取详情（私有）
   * @param id
   * @param config
  ...
  async _getInfo(
    ...
    const item = await ExpressDeliveryDb.findOne({
      where,
    });
    if (!item) {
      if (autoReturnError) {
  ...
  async create(data: any): Promise<ManagerResponse<any>> {
    ...
    const result = await ExpressDeliveryDb.create(data);
    if (result) {
  ...
  async edit(
    ...
    const result = await ExpressDeliveryDb.update(updateData, {
      where: {
        id,
      },
    });
  ...
  async del(id: number): Promise<ManagerResponse<any>> {
    ...
    return await sequelize.transaction(async (t: any) => {
      const result = await ExpressDeliveryDb.destroy({
        where: {
          id,
        },
      });
  ...
  async getList?(
    ...
    return await sequelize.transaction(async (t: any) => {
      ...
      const result = await ExpressDeliveryDb.findAndCountAll({
        ...listParams,
      });
      const { count, rows } = result;
      const ExpressDeliveryList = rows.map((row: any) => {
...
export default ExpressDeliveryManager;
