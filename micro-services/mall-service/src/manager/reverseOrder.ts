...
import ReverseOrderDb from "@micro-services/mall-service/src/db/reverseOrder";

const placeholder = "ReverseOrder";
const responseMsg = ResponseMsg(placeholder);
class ReverseOrderManager implements CommonManager {
  /**
   * 获取详情（私有）
   * @param id
   * @param config
  ...
  async _getInfo(
    ...
    const item = await ReverseOrderDb.findOne({
      where,
    });
    if (!item) {
      if (autoReturnError) {
  ...
  async create(data: any): Promise<ManagerResponse<any>> {
    ...
    const result = await ReverseOrderDb.create(data);
    if (result) {
  ...
  async edit(
    ...
    const result = await ReverseOrderDb.update(updateData, {
      where: {
        id,
      },
    });
  ...
  async del(id: number): Promise<ManagerResponse<any>> {
    ...
    return await sequelize.transaction(async (t: any) => {
      const result = await ReverseOrderDb.destroy({
        where: {
          id,
        },
      });
  ...
  async getList?(
    ...
    return await sequelize.transaction(async (t: any) => {
      ...
      const result = await ReverseOrderDb.findAndCountAll({
        ...listParams,
      });
      const { count, rows } = result;
      const ReverseOrderList = rows.map((row: any) => {
...
export default ReverseOrderManager;
