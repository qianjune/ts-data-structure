...
class CodeRouter extends BaseRouter {
  ...
  async sendCodeToMobileForResetPassword(ctx: Context): Promise<void> {
    ...
    await CodeService.sendCodeByMobile(
      user,
      action,
      (captcha as string).toLowerCase()
    );
  }
  ...
  async sendCodeForMobile(ctx: Context): Promise<void> {
    ...
    await CodeService.sendCodeByMobile(
      user,
      CODE_ACTION_TYPE.REGISTER_AND_LOGIN
    );
  }
  ...
  async validateCodeForMobile(ctx: Context): Promise<void> {
    ...
    await CodeService.validateCodeByMobile(
      user,
      CODE_ACTION_TYPE.REGISTER_AND_LOGIN,
      code
    );
  ...
  async validateCodeForEmail(ctx: Context): Promise<void> {
    const { user, code } = ctx.request.body;
    await CodeService.validateCodeByEmail(
      user,
      CODE_ACTION_TYPE.REGISTER,
      code
    );
  }
}
