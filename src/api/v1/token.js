...
router.post('/verify', async (ctx) => {
  ...
  const result = await Auth.verifyToken(v.get('body.token'), global.config.security.secretKey)
  ctx.body = {
    result
  }
})
...
const genNewToken = (id, scope) => {
  ...
  const accessToken = generateToken(security.secretKey, security.expiresIn)(id, scope)
  const refreshToken = generateToken(refresh.secretKey, refresh.expiresIn)(id, scope)
  return {
    accessToken,
    refreshToken
  }
