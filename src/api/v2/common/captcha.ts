...
import BaseRouter, { prefix, tag, get, summary, parameter } from "@src/lib/router-decorator";
...
class CaptchaApi extends BaseRouter {
  ...
  async buildCaptcha(ctx: Context) {
    ...
    await ValidateCodeModel.saveCode({ user, key: type + '-' + 'captcha', code: text.toLowerCase() })
    ResponseHandler.send(new ManagerResponseSuccess({ msg: 'svg验证码', data }))
  }
}
